...
class AnimeBoomParser(BasicAnimeParser):
        ...
        return AniBoomAnime(
            **base_info.__dict__,
            synonyms=self._find_synonyms(
                BeautifulSoup(html_code, self.engine)
            )
        )
    ...
    def _find_info(self, soup: BeautifulSoup):
        ...
        for dt, dd in zip(dt, dd):
            key = ''.join([i.get_text(strip=True) for i in dt.children if len(i.get_text(strip=True)) > 1])
            value = [i.get_text(strip=True) for i in dd.children if len(i.get_text(strip=True)) > 1]
            
            info.append(
                AnimeRow(
                    key,
                    ''.join(value) if len(value) == 1 else value,
